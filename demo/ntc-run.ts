...
function mcuTagSetter(mcuNumber: number) {
  return (coreNumber: number, pc: number, tag: string) => {
    mcu_tags[mcuNumber][coreNumber] = tag;
    if (tag.startsWith('!')) {
      console.log(
...
function getVarOffs(mcu_id: number, var_name: string): number {
  ...
  if (res == null)
    throw new Error(`Could not find offset of variable ${var_name} in map file ${filename}`);
...
let pin_state_inp: number[][] = new Array(hex_files.length)
  .fill(null)
  .map(() => new Array(pin_gpio.length).fill(3)); // all start in input pullup mode
...
function exactPinTick() {
  ...
  for (let i = 0; i < pin_label.length; i++) {
    ...
    if (v_old != v_new) {
      //xxx TODO GPIO outputs should probably read back their output as input without latency, but this eats a lot of emulator performance
      const tfv = (v_new & 0b01) == 0; // any single Low will pull down bus
      const gpio_pin = pin_gpio[i];
      for (let mcu_id = 0; mcu_id < hex_files.length; mcu_id++)
        mcu[mcu_id].gpio[gpio_pin].setInputValue(tfv);
    }
  ...
  return (state: GPIOPinState, oldState: GPIOPinState) => {
    ...
    if (vcd_enabled) {
      ...
      const conflict: boolean =
        (pin_state_inp[0][pin] === 0 && pin_state_inp[1][pin] === 1) ||
        (pin_state_inp[0][pin] === 1 && pin_state_inp[1][pin] === 0);
...
async function write_pic(filename: string) {
  ...
  const height = 300;
  const palette = [
    0x00, 0xff, 0x84, 0x7b, 0x86, 0x55, 0x26, 0xfd, 0x88, 0x44, 0xcd, 0x49, 0x6d, 0xbe, 0x6f, 0xb6,
  ];
...
async function run_mcus() {
  ...
  function log_state() {
    ...
    let wTags: string[] = [];
    const pTags_updated: string[] = [
      mcu_tags[0][0],
      mcu_tags[0][1],
      mcu_tags[1][0],
      mcu_tags[1][1],
      mcu_main.pio[0].machines[0].pc.toString(),
      mcu_vic.pio[1].machines[0].pc.toString(),
      mcu_vic.pio[1].machines[1].pc.toString(),
      mcu_output.pio[1].machines[0].pc.toString(),
    ];
    const pInstrs: number[] = [
      0,
      0,
      0,
      0,
      mcu_main.pio[0].instructions[mcu_main.pio[0].machines[0].pc],
    ...
    let bus_bin = 0;
    for (let i = 8; i > 0; i--) {
      let bus_pin = mcu_output.gpio[pin_gpio[i]].inputValue >>> 0;
      bus_bin = (bus_bin << 1) + bus_pin;
      bus_pins = bus_pins + bus_pin.toString();
    }
    ...
    logs.push(
      `${cycleTag} / ${busTag} | ${bus_state_str} | M ${mcu_main.core0.PC.toString(16).padStart(
        ...
        wTags[2]
      } ${mcu_vic.core1.PC.toString(16).padStart(8, '0')}/${wTags[3]} | M_PIO@${wTags[4]} V_PIO@${
        wTags[5]
      }/r${mcu_vic.pio[1].machines[0].rxFIFO.itemCount}/t${
        mcu_vic.pio[1].machines[0].txFIFO.itemCount
      } V_OUT@${wTags[6]} O_INP@${wTags[7]} | V_H_COUNT@${vic_h
        .toString()
  ...
  try {
    for (let i = 0; i < 1000000; i++) {
        ...
        while (mcu_cycles_behind[mcu_id] > 0) {
          ...
          if (mcu_id == 1) {
            ...
            if (vic_cycle_state != 0 && mcu_tags[1][0] == '^vic tick') {
              ...
              vic_loop_stats.push({
                startCycle: vic_cycle_start_at,
                duration: mcu[1].core0.cycles - vic_cycle_start_at,
                vic_h: vic_h,
                vic_l: vic_l,
      ...
      if (main_cycle_start_off == 0 && mcu_tags[0][0] == 'cycle start') {
        ...
        main_loop_stats.push({
          startCycle: main_cycle_start_at,
          duration: mcu[0].core0.cycles - main_cycle_start_at,
          idle: main_idle_cycles,
          idle2: main_idle2_cycles,
          vic_h: vic_h,
          vic_l: vic_l,
          addr6510: mcu[0].readUint16(cpu_addr_off),
          cycle6510: cycles_6510,
        });
        cycles_6510++;
        main_idle_cycles = 0;
        main_idle2_cycles = 0;
        if (main_loop_stats.length > 100000)
          main_loop_stats = main_loop_stats.slice(main_loop_stats.length - max_len_main_loop_stats);
        vic_h++;
        if (vic_h > 62) {
          vic_h = 0;
          vic_l++;
      ...
      if (do_tracing) {
        log_state();
        if (debug_crash_cycle > 0 && mcu[0].core0.cycles > debug_crash_cycle)
          throw new Error('Debug end tracing');
      } else {
        if (debug_crash_cycle > 0 && mcu[0].core0.cycles > debug_crash_cycle - 10000)
          do_tracing = true;
        if (debug_trace_from_emu_cycle > 0 && cycles_6510 > debug_trace_from_emu_cycle) {
          do_tracing = true;
          debug_crash_cycle = mcu[0].core0.cycles + 4200;
        }
        if (
          debug_trace_from_emu_addr > 0 &&
          mcu[0].readUint16(cpu_addr_off) == debug_trace_from_emu_addr
        ) {
          do_tracing = true;
          debug_crash_cycle = mcu[0].core0.cycles + 4200;
        }
      ...
      if (trace_6510_file || trace_6510_file_it) {
        ...
        if (addr_6510 != addr_6510_last) {
          ...
          if (trace_6510_file) {
            ...
            } else if (Number(`0x${line.value}`) != addr_6510)
              throw new Error(
                `6510 addr mismatch, expected ${line.value}, got ${addr_6510
                  .toString(16)
                  .padStart(
                    4,
                    '0'
    ...
    console.error('\n*** 6510 statistics ***');
    if (main_loop_stats.length > max_len_main_loop_stats)
      main_loop_stats = main_loop_stats.slice(main_loop_stats.length - max_len_main_loop_stats);
    for (let l of main_loop_stats) {
      console.error(
        `6510 cycle ${l.cycle6510}, ARM cycle ${l.startCycle}, MAIN total/idle ${l.duration}/${
          l.idle
    ...
    console.error('\n*** VIC-II statistics ***');
    if (vic_loop_stats.length > max_len_vic_loop_stats)
